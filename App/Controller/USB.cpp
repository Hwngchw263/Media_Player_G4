...
std::string USB::getUsername() const {
    ...
    for (const auto &entry : fs::directory_iterator(basePath)) {
        if (entry.is_directory()) {
            std::string usbName = entry.path().filename().string();
            if (usbName != "EVB-S32K144") {
...
std::vector<std::string>& USB::getUSBPaths() {
    usbPaths.clear();
    std::string basePath = "/media/" + username;
    std::regex evbPattern("^EVB-S32K144.*"); // Regular expression to match "EVB-S32K144" followed by any characters

    for (const auto &entry : fs::directory_iterator(basePath)) {
        if (entry.is_directory()) {
            std::string dirName = entry.path().filename().string();
            if (!std::regex_match(dirName, evbPattern)) { // If the directory name does not match the pattern
